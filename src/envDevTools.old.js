...
let startdevTools //入口
let sendMsg // 对外暴露的上报数据的方法
...
!(function () {
   ...
   */
  startdevTools = (options = newOptions) => {
    Object.assign(newOptions, options)  //覆盖配置
    checkOptions(newOptions)  //校验options

    newOptions.watchError && preWatchError()  //前置监听 
  ...
  const checkOptions = (options) => {
    ...
    for (const key in options) {
      ...
      if (val == null) {
        throw new Error("options exist null or undefined")
      }
    }
  }
  ...
  const createEnvDevTools = (options) => {
    let { envBoxIdName, envBoxExpandIdName, insertDOM, watchPerformance, watchError } = options
